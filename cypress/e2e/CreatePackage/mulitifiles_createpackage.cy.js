describe('CreatePackage_Uploadfile',() => {
    it("UploadFIle_Ingestion",()=>  {
        ...
        cy.get("#mat-input-8").type("auto_package_203")
        ...
        cy.get('#fileDropRef').selectFile("cypress\\fixtures\\Sourcefile\\SWIFT_FILE_DATA.csv");
        cy.get('#fileDropRef').selectFile("cypress\\fixtures\\Sourcefile\\Warehouse_Stock_Data.csv");
        //cy.readFile('cypress\\fixtures\\Sourcefile\\wwi2.csv') 
        //Ingestion COnfig
        cy.get('#mat-input-9').type("TYPE,ARCHITECTURE_LAYER,FILE_NAME,SOURCE_NAME,ACTION,KEY,VALUE \nENV,ALL,ALL,DATA360,ROOT_PATH,BUCKET_NAME,/.datalake \nENV,ALL,ALL,DATA360,CORE_KEY,QUALITY_RATING_COL_NAME,col_name=DL_QUALITY_RATING1 || col_value=0 \n,,,,,, \nPRO,L0,PARTY_7KB,DATA360,Ingestion,INPUT_DETAILS,path=/mock_source/Warehouse_Stock_Data.csv || file_format = csv || header=true || sep=NA || table_name=NA \nPRO,L0,PARTY_7KB,DATA360,Ingestion,OUTPUT_DETAILS,path=/output/Ingestion/Warehouse_Stock_Data/ || file_format = parquet || header=true || sep=NA || table_name=NA || notify_to= dharma.koppisetty@amnetdigital.com || notification_message=Ingestion has been run succesfully for ware house stock data. Please check the results \n\n,,,,,, \n\nPRO,L0,WWI,DATA360,Ingestion,INPUT_DETAILS,path=/mock_source/SWIFT_FILE_DATA.csv || file_format = csv || header=true || sep=NA || table_name=NA \nPRO,L0,WWI,DATA360,Ingestion,OUTPUT_DETAILS,path=/output/Ingestion/SWIFT_FILE_DATA/ || file_format = parquet || header=true || sep=NA || table_name=NA || notify_to= dharma.koppisetty@amnetdigital.com || notification_message=Ingestion has been run succesfully for swift data. Please check the result")
