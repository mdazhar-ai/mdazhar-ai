describe('CreatePackage_Uploadfile',() => {
    it("UploadFIle_Ingestion",()=>  {
        ...
        cy.get("#mat-input-8").type("auto_package_237")
        ...
        cy.wait(2000)
        cy.get('#mat-button-toggle-6-button > .mat-button-toggle-label-content').click()
        cy.get('#mat-input-22').type("TYPE,ARCHITECTURE_LAYER,FILE_NAME,SOURCE_NAME,ACTION,KEY,VALUE\nENV,ALL,ALL,DATA360,ROOT_PATH,BUCKET_NAME,/.datalake\nENV,ALL,ALL,DATA360,CORE_KEY,QUALITY_RATING_COL_NAME,col_name=DL_QUALITY_RATING1 || col_value=0\n\"\"\"\"\nPRO,L0,SI,DATA360,Collections,INPUT_DETAILS,path=/output/DataQuality/wwi2/ || file_format = parquet || header=true || sep=NA || table_name=NA\nPRO,L0,SI,DATA360,Collections,identity_table2,\"rule_name = identity_table || table_name=/output/Collections/CSV12/DimCustomer || identity_table_columns=CompanyName || identity_input_column=Supplier_Party || columns=CompanyName || values=Supplier_Party || auto_inc=NA\"\nPRO,L0,SI,DATA360,Collections,OUTPUT_DETAILS,path=NA || file_format = csv || header=true || sep=NA || table_name=NA")
        cy.get('form.ng-star-inserted > :nth-child(2) > .mat-focus-indicator').click()
        cy.get('.save-btn').click()
        cy.get('#mat-dialog-1').scrollIntoView('#mat-dialog-title-1')
        cy.get('#mat-dialog-title-1').contains("summary").should('be.visible')
