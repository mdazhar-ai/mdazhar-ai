describe('Pointtotarget',() => {
    ...
    it("MySQL_Connection",()=>  {
        ...
        cy.get('#serverName').type("ec2-43-204-54-237.ap-south-1.compute.amazonaws.com")
        cy.get('#databaseName').type("shedulertestfromsi")
        cy.get('#userName').type("mysql_azhar")
        cy.get('#password').type("mysql_azhar")
        cy.get('.col > .mat-focus-indicator > .mat-button-wrapper').click()
        cy.get(':nth-child(4) > .align-items-center').contains("Connection Successful")

    })
    it("Postgresql_Connection",()=>  {
        ...
        cy.get('#serverName').type("ec2-43-204-54-237.ap-south-1.compute.amazonaws.com")
        cy.get('#databaseName').type("shedulertestfromsi")
        cy.get('#userName').type("Azhar_postgres_dev")
        cy.get('#password').type("Azhar_postgres_dev**&*&^^^&&%%3314556")
        cy.get('.col > .mat-focus-indicator > .mat-button-wrapper').click()
        cy.get(':nth-child(4) > .align-items-center').contains("Connection Successful")

    })
    it("Snowflake_Connection",()=>  {
        ...
        cy.get('#serverName').type("ohhbczt-ap05302")
        cy.get('#databaseName').type("SDDATABASE")
        cy.get('#userName').type("VENOMAZHAR13")
        cy.get('#password').type("Icon@1234")
        cy.get('#schema').type("SUPERSTOREFILE")
        cy.get('#warehouse').type("COMPUTE_WH")
        cy.get('.col > .mat-focus-indicator > .mat-button-wrapper').click()
        cy.get(':nth-child(4) > .align-items-center').contains("Connection Unsuccessful")
